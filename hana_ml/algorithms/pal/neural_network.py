...
class _MLPBase(PALBase):#pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    def __init__(self,#pylint: disable=too-many-arguments, too-many-branches, too-many-statements, too-many-locals
                 ...
                 param_range=None,
                 thread_ratio=None):
        super(_MLPBase, self).__init__(conn_context)
        #type checking
        ...
        if hidden_layer_size is not None:
            if isinstance(hidden_layer_size, tuple) and all(isinstance(x, _INT_TYPES) for x in hidden_layer_size):#pylint:disable=line-too-long
                self.hidden_layer_size = ','.join(str(x) for x in hidden_layer_size)
            else:
                msg = "Parameter 'hidden_layer_size' must be type of tuple of int."
                logger.error(msg)
                raise TypeError(msg)
        else:
    ...
    def _fit(self, data, key=None, features=None, label=None, categorical_variable=None):#pylint: disable=too-many-locals, too-many-statements, too-many-branches
        ...
        try:
            ...
            self._call_pal_auto('PAL_MULTILAYER_PERCEPTRON',
                                training_df,
        ...
        self.stats_ = self.conn_context.table(stat_tbl)#pylint:disable=attribute-defined-outside-init
        self.train_log_ = self.conn_context.table(log_tbl)#pylint:disable=attribute-defined-outside-init
        self.optim_param_ = self.conn_context.table(optimal_tbl)#pylint:disable=attribute-defined-outside-init
    ...
    def _predict(self, data, key, features=None, thread_ratio=None):#pylint: disable=too-many-locals
        ...
        try:
            self._call_pal_auto('PAL_MULTILAYER_PERCEPTRON_PREDICT',
                                data,
                                self.model_,
        ...
        return (self.conn_context.table(result_tbl),
                self.conn_context.table(soft_max_tbl))
...
class MLPClassifier(_MLPBase):
    ...
    ...                      thread_ratio=0.3)
    >>> mlpc.fit(df, label='LABEL', categorical_variable='V003')
    ...
    Paramete selection result may look different from the following result due to randomness.
    ...
    def __init__(self, conn_context, activation=None, activation_options=None,
        ...
        super(MLPClassifier, self).__init__(conn_context=conn_context,
...
class MLPRegressor(_MLPBase):
    ...
    def __init__(self, conn_context, activation=None, activation_options=None,
                 ...
                 random_state=None, timeout=None, progress_indicator_id=None,
                 param_values=None, param_range=None, thread_ratio=None):
        super(MLPRegressor, self).__init__(conn_context=conn_context,
                                           activation=activation,
                                           activation_options=activation_options,
                                           output_activation=output_activation,
                                           output_activation_options=output_activation_options,
