"""
This module contains PAL wrapper for clustering algorithms.
...
class _ClusterAssignmentMixin(object):#pylint: disable=bad-option-value
    ...
    def _predict(self, data, key, features=None):
        ...
        try:
            ...
            self._call_pal_auto('PAL_CLUSTER_ASSIGNMENT',
                                adjusted_data,
                                self.model_,
                                param_t,
                                assignment_tbl)
            ...
            self._try_drop(assignment_tbl)
            raise

        return self.conn_context.table(assignment_tbl)
...
class AffinityPropagation(PALBase):#pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, conn_context,
        ...
        super(AffinityPropagation, self).__init__(conn_context)
    ...
    def fit(self, data, key, features=None):#pylint: disable=too-many-locals
        ...
        with self.conn_context.connection.cursor() as cur:
            ...
            seed = self.conn_context.table(seed_tbl)

        try:
            param_t = ParameterTable(name='#PARAM_TBL')
            self._call_pal_auto('PAL_AFFINITY_PROPAGATION',
                                data,
                                seed,
        ...
        self.labels_ = self.conn_context.table(result_tbl)#pylint:disable=attribute-defined-outside-init
...
class AgglomerateHierarchicalClustering(PALBase):#pylint: disable=too-many-instance-attributes
    ...
    >>> hc.fit(df, key='POINT', categorical_variable=['X3'])
    ...
    def __init__(self,
        ...
        super(AgglomerateHierarchicalClustering, self).__init__(conn_context)
        self.n_clusters = self._arg('n_clusters', n_clusters, int)
        self.affinity = self._arg('affinity', affinity, self.affinity_map)
    ...
    def fit(self, data, key, features=None, categorical_variable=None):#pylint: disable=too-many-locals
        ...
        try:
            param_t = ParameterTable(name='#PARAM_TBL')
            self._call_pal_auto('PAL_HIERARCHICAL_CLUSTERING',
                                data,
                                param_t.with_data(param_rows),
                                *outputs)
        except dbapi.Error as db_err:
        ...
        self.combine_process_ = self.conn_context.table(combine_process_tbl)#pylint:disable=attribute-defined-outside-init
        self.labels_ = self.conn_context.table(result_tbl)#pylint:disable=attribute-defined-outside-init
    ...
    def fit_predict(self, data, key, features=None, categorical_variable=None):
        ...
        DataFrame
            ...
            combine_process: structured as follows:
            ...
            label: label of each points, structured as follows:
...
class DBSCAN(PALBase, _ClusterAssignmentMixin):#pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, conn_context, minpts=None, eps=None, thread_ratio=None,#pylint: disable=too-many-arguments
                 metric=None, minkowski_power=None, categorical_variable=None,
                 category_weights=None, algorithm=None, save_model=True):
        super(DBSCAN, self).__init__(conn_context)
    ...
    def fit(self, data, key, features=None, categorical_variable=None):#pylint: disable=too-many-locals
        ...
        try:
            param_t = ParameterTable(name='#PARAM_TBL')
            self._call_pal_auto('PAL_DBSCAN',
                                data[[key] + features],
                                param_t.with_data(param_rows),
                                *outputs)
        except dbapi.Error as db_err:
        ...
        self.labels_ = self.conn_context.table(result_tbl)#pylint:disable=attribute-defined-outside-init
        self.model_ = self.conn_context.table(model_tbl) if self.save_model else None#pylint:disable=attribute-defined-outside-init
...
class GeometryDBSCAN(PALBase):#pylint: disable=too-many-instance-attributes
    """
    rThis function is a geometry version of DBSCAN, which only accepts geometry points as input data.
    ...
    In HANA, the test table PAL_GEO_DBSCAN_DATA_TBL can be created by the following SQL:
    ...
    >>> geo_dbscan = GeometryDBSCAN(conn, thread_ratio=0.2, metric='manhattan')

    Perform fit on the given data:

    >>> geo_dbscan.fit(df, key='ID')
    ...
    def __init__(self, conn_context, minpts=None, eps=None, thread_ratio=None,#pylint: disable=too-many-arguments
        ...
        super(GeometryDBSCAN, self).__init__(conn_context)
        self.auto_param = 'true' if any(param is None for param in (minpts, eps)) else 'false'
        if (minpts is not None and eps is None) or (minpts is None and eps is not None):
            msg = 'minpts and eps need to be provided together.'
            logger.error(msg)
    ...
    def fit(self, data, key, features=None):
        ...
        try:
            param_t = ParameterTable(name='#PARAM_TBL')
            self._call_pal_auto('PAL_GEO_DBSCAN',
                                data[[key] + features],
                                param_t.with_data(param_rows),
                                *outputs)
        except dbapi.Error as db_err:
...
class KMeans(PALBase, _ClusterAssignmentMixin):#pylint: disable=too-many-instance-attributes
    ...
    thread_ratio : float, optional
        ...
        Controls the proportion of available threads to use.
        The value range is from 0 to 1, where 0 indicates a single thread,
        and 1 indicates up to all available threads. Values between 0
        and 1 will use up to that percentage of available threads. Values
        outside this range tell PAL to heuristically determine the number
        of threads to use.
    ...
    def __init__(self, conn_context, n_clusters=None, n_clusters_min=None, n_clusters_max=None,#pylint: disable=too-many-branches, too-many-locals, too-many-statements, too-many-arguments
        ...
        super(KMeans, self).__init__(conn_context)
...
class _KClusteringBase(PALBase):#pylint: disable=too-many-instance-attributes
    ...
    def __init__(self,#pylint: disable=too-many-arguments, too-many-branches
        ...
        super(_KClusteringBase, self).__init__(conn_context)
    ...
    def fit(self, data, key, features=None, categorical_variable=None):#pylint: disable=invalid-name, too-many-locals
        ...
        try:
            param_t = ParameterTable(name='#PARAM_TBL')
            self._call_pal_auto(self._proc_name,
                                data,
            ...
            self._try_drop(centers_tbl)
            raise

        self.cluster_centers_ = self.conn_context.table(centers_tbl)
...
class KMedians(_KClusteringBase):
    ...
    thread_ratio : float, optional
        ...
        between 0 and 1 will use up to that percentage of available threads.
