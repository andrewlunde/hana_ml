"""
This module contain PAL wrapper for the PAL partition function.
...
def train_test_val_split(conn_context, data, random_seed=None, thread_ratio=None,
    ...
    train_df : DataFrame
        New DataFrame with select statement that calculates training data.
        Table structure identical to input data table.
    test_df : DataFrame
        New DataFrame with select statement that calculates testing data.
        Table structure identical to input data table.
    validation_df : DataFrame
        New DataFrame with select statement that calculates validation data.
        Table structure identical to input data table.
    """
    # SQLTRACE
    conn_context.sql_tracer.set_sql_trace(None, 'partition', 'train_test_val_split')
    ...
    train_sql = 'SELECT a.* FROM {} a inner join {} b \
       on a.{} = b.{} where b."PARTITION_TYPE" = 1' .format(data_tbl, result_tbl,
                                                            ...
                                                            quotename(id_column_name))
    train_ = dataframe.DataFrame(conn_context, train_sql)#pylint:disable=attribute-defined-outside-init
     # SQLTRACE
    conn_context.sql_tracer.trace_object({
        'name': 'train',
    ...
    test_sql = 'SELECT a.* FROM {} a inner join {} b \
       on a.{} = b.{} where b."PARTITION_TYPE" = 2' .format(data_tbl, result_tbl,
                                                            quotename(id_column_name),
                                                            quotename(id_column_name))
    test_ = dataframe.DataFrame(conn_context, test_sql)#pylint:disable=attribute-defined-outside-init
     # SQLTRACE
    conn_context.sql_tracer.trace_object({
        'name': 'test',
    ...
    validation_sql = 'SELECT a.* FROM {} a inner join {} b \
       on a.{} = b.{} where b."PARTITION_TYPE" = 3' .format(data_tbl, result_tbl,
                                                            quotename(id_column_name),
                                                            quotename(id_column_name))
    validation_ = dataframe.DataFrame(conn_context, validation_sql)#pylint:disable=attribute-defined-outside-init
     # SQLTRACE
    conn_context.sql_tracer.trace_object({
        'name': 'validation',
