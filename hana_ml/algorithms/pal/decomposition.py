...
#pylint: disable=too-many-locals, line-too-long, too-many-arguments, too-many-lines
import logging
import uuid
...
class PCA(PALBase):#pylint: disable=too-many-instance-attributes
    r"""
    ...
    def __init__(self,#pylint: disable=too-many-arguments
                 ...
                 thread_ratio=None,
                 scores=None):
        super(PCA, self).__init__(conn_context)
    ...
    def fit_transform(self, data, key, features=None, label=None):#pylint:disable=invalid-name
        ...
        if self.scores_ is None:
            return self.transform(data, key, features, label)
        if label is None:
            return self.scores_
    ...
    def transform(self, data, key, features=None, n_components=None, label=None):#pylint:disable=invalid-name, too-many-locals
        ...
        DataFrame
            ...
            structured as follows:
                ...
                values of each data point.
        """
        if self.loadings_ is None or self.scaling_stat_ is None:
        ...
        try:
            param_t = ParameterTable(name='#PARAM_TBL')
            self._call_pal_auto('PAL_PCA_PROJECT',
                                data,
                                self.loadings_,
                                self.scaling_stat_,
                                param_t.with_data(param_rows),
            ...
            logger.exception(str(db_err))
            self._try_drop('#PARAM_TBL')
            self._try_drop(scores_tbl)
            raise

        return self.conn_context.table(scores_tbl)
...
class LatentDirichletAllocation(PALBase):#pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, conn_context, n_components, doc_topic_prior=None, topic_word_prior=None,#pylint: disable=too-many-arguments
        ...
        super(LatentDirichletAllocation, self).__init__(conn_context)
    ...
    def fit(self, data, key, document=None):#pylint: disable=too-many-locals
        ...
        try:
            param_t = ParameterTable(name='#PARAM_TBL')
            self._call_pal_auto('PAL_LATENT_DIRICHLET_ALLOCATION',
                                data,
                                param_t.with_data(param_rows),
                                *outputs)
        except dbapi.Error as db_err:
        ...
        # pylint: disable=attribute-defined-outside-init
        self.doc_topic_dist_ = self.conn_context.table(doc_top_dist_tbl)
        if self.output_word_assignment:
            self.word_topic_assignment_ = self.conn_context.table(word_topic_assignment_tbl)
        else:
            self.word_topic_assignment_ = None
        if any(x is not None for x in (self.threshold_top_words, self.max_top_words)):
            self.topic_top_words_ = self.conn_context.table(topic_top_words_tbl)
        else:
            self.topic_top_words_ = None
        self.topic_word_dist_ = self.conn_context.table(topic_word_dist_tbl)
        self.dictionary_ = self.conn_context.table(dict_tbl)
        self.statistic_ = self.conn_context.table(stat_tbl)
        self._cv_param = self.conn_context.table(cv_param_tbl)
    ...
    def transform(self, data, key, document=None, burn_in=None, #pylint: disable=too-many-arguments, too-many-locals
        ...
        #dictionary, cv_param
        if not all(hasattr(self, name) for name in ('topic_word_dist_', 'dictionary_',
                                                    '_cv_param')):
        ...
        try:
            param_t = ParameterTable(name='#PARAM_TBL')
            self._call_pal_auto('PAL_LATENT_DIRICHLET_ALLOCATION_INFERENCE',
                                data,
                                self.topic_word_dist_,
                                self.dictionary_,
                                self._cv_param,
                                param_t.with_data(param_rows),
                                *outputs)
        ...
        return (self.conn_context.table(doc_top_dist_tbl),
                self.conn_context.table(word_topic_assignment_tbl) if output_word_assignment
                else None,
                self.conn_context.table(stat_tbl))
